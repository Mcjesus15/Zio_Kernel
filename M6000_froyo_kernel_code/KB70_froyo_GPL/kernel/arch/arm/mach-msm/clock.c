...
//[KB62] ===> BugID#645 : Froyo porting : turn off TCXO during CPU idle, added by Jimmy@CCI
#ifdef CONFIG_CCI_TURN_OFF_TCXO_DURING_CPU_IDLE
int idle_disabled_clks[NR_CLKS];
...
#ifdef CONFIG_CCI_TURN_OFF_TCXO_DURING_CPU_IDLE_DEBUG
	idle_disabled_clks_index = 0;
#endif // #ifdef CONFIG_CCI_TURN_OFF_TCXO_DURING_CPU_IDLE_DEBUG
	for(i = 0; i < NR_CLKS; i++)
	{
		//ignore the following clocks, turn them off by us
		//refer to clock-pcom.h
		if(		i == 1		//P_ADM_CLK
			||	i == 9		//P_I2C_CLK
			||	i == 14	//P_MDP_CLK
			||	i == 17	//P_PMDH_CLK
...
#ifdef CONFIG_CCI_TURN_OFF_TCXO_DURING_CPU_IDLE_DEBUG
	if(idle_disabled_clks_index>0)
	{
		printk("[kernel]cci_check_all_enabled_clock() : ");
		for(i=0; i<idle_disabled_clks_index; i++)
		{
			printk("%d ", idle_disabled_clks[i]);
		}
		printk("\n");
	}
#endif // #ifdef CONFIG_CCI_TURN_OFF_TCXO_DURING_CPU_IDLE_DEBUG
...
//re-enable clocks that disabled
