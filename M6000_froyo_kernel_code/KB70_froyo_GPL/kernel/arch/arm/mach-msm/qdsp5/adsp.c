...
#include <mach/msm_battery.h>    //weijiun_ling
#include <linux/string.h>    //weijiun_ling
		 ...
		 struct msm_adsp_ops *ops, void *driver_data)
{
	...
	MM_INFO("opening module %s\n", module->name);
        ...
        printk("adsp: opening module %s , length =%d\n", module->name,strlen(module->name));//bincent
        if (strlen(module->name) == 9)
            cci_batt_device_status_update(CCI_BATT_DEVICE_ON_VIDEO_50,1);
      #endif   
...
void msm_adsp_put(struct msm_adsp_module *module)
{
	...
	if (module->ops) {
		MM_INFO("closing module %s\n", module->name);
             ...
             //weijiun_ling
             #ifdef NOTIFY_BATT__FUNCTION
                printk("adsp: closing module %s , length =%d\n", module->name,strlen(module->name));//bincent
                if (strlen(module->name) == 9)
                    cci_batt_device_status_update(CCI_BATT_DEVICE_ON_VIDEO_50,0); 
             #endif
