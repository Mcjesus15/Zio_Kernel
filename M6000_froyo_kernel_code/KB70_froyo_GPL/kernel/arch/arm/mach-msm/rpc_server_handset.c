...
#define RPC_DETECT_BATTERY_REMOVE_PROC 4 //bincent
...
#define HS_DIAG_HOME		0x12
#define HS_DIAG_MENU		0x13
#define HS_DIAG_BACK		0x52
#define HS_DIAG_SEARCH		0x14
#define HS_DIAG_TALK		0x50
#define HS_DIAG_END		0x51
#define HS_DIAG_UP		0x54
#define HS_DIAG_DOWN		0x55
#define HS_DIAG_RIGHT		0x0A
#define HS_DIAG_LEFT		0x09
#define HS_DIAG_OK		0x0C
#define HS_DIAG_VOL_UP		0x8F
#define HS_DIAG_VOL_DN		0x90
#define HS_DIAG_CAMERA		0xA2
#define HS_DIAG_FOCUS           0xD2
...
static const uint32_t hs_key_map[] = {
	...
	KEY(HS_DIAG_HOME,   KEY_HOME),
	KEY(HS_DIAG_MENU,   KEY_MENU),
	KEY(HS_DIAG_BACK,   KEY_BACK),
	KEY(HS_DIAG_SEARCH, KEY_SEARCH),
	KEY(HS_DIAG_TALK,   KEY_SEND),
	KEY(HS_DIAG_END,    KEY_F4), //KEY_END
	KEY(HS_DIAG_UP,     KEY_UP),
	KEY(HS_DIAG_DOWN,   KEY_DOWN),
	KEY(HS_DIAG_RIGHT,  KEY_RIGHT),
	KEY(HS_DIAG_LEFT,   KEY_LEFT),
	KEY(HS_DIAG_OK,     KEY_ENTER),
	//KEY(HS_HEADSET_SWITCH_2_K, KEY_VOLUMEUP),
	//KEY(HS_HEADSET_SWITCH_3_K, KEY_VOLUMEDOWN),
	KEY(HS_DIAG_VOL_UP, KEY_VOLUMEUP),
	KEY(HS_DIAG_VOL_DN, KEY_VOLUMEDOWN),
	KEY(HS_DIAG_CAMERA, KEY_CAMERA),
	KEY(HS_DIAG_FOCUS,  KEY_HP),
	0
};
...
struct input_dev *tskpdev = NULL;
#endif

static struct work_struct diag_work;
static wait_queue_head_t diag_wait;
static bool diag_stop_wait = false;
static uint32_t previous_key = 0;
static void diag_cmd_func (struct work_struct *no_use)
{
	wait_event_timeout(diag_wait, diag_stop_wait, msecs_to_jiffies(1200));
	input_report_key(hs->ipdev, previous_key, 0);
	input_sync(hs->ipdev);
	printk(KERN_INFO "%s: DIAG_CMD keycode: %d, %s press\n",
		__func__, previous_key, diag_stop_wait ? "short" : "long ");
}
...
static void report_hs_key(uint32_t key_code, uint32_t key_parm)
{
   ...
   static int headset_on = 0;
	...
	//case KEY_VOLUMEDOWN:
		...
		cci_key_handler(key, (key_code != HS_REL_K)?1:0);
		break;
	case SW_HEADPHONE_INSERT:
		printk("%s:  SW_HEADSET event : %d\n", __func__, (key_code != HS_REL_K));
		report_headset_switch(hs->ipdev, key, (key_code != HS_REL_K));
		headset_on = (key_code != HS_REL_K);
		break;
	case KEY_MEDIA:
                printk("%s:  KEY_MEDIA event : %d\n", __func__, (key_code != HS_REL_K));
		// cci.johnny_lee, disable long-press function to avoid wrong event from ARM9
		if (headset_on) {
                	if (key_code == HS_REL_K) {
                		input_report_key(hs->ipdev, key, 1);
                		input_report_key(hs->ipdev, key, 0);
                	}
		} else {
                	printk("%s:  wrong KEY_MEDIA event from ARM9 \n", __func__);
		}
		break;
	case KEY_HOME:
	case KEY_MENU:
	...
	case KEY_CAMERA:
	case KEY_HP:
		if (key_code != HS_REL_K) { //press key
			printk("%s:  Key pressed = 0x%x\n", __func__, temp_key_code);
			input_report_key(hs->ipdev, key, 1);
			previous_key = key;
			diag_stop_wait = false;
			   ...
			   struct rpc_request_hdr *req, unsigned len)
{
	...
	case RPC_KEYPAD_SET_PWR_KEY_STATE_PROC:
		 ...
		 */
		return 0;
	case RPC_DETECT_BATTERY_REMOVE_PROC:
		printk("Removal Battery Caused Power down.\n");
...
static int __devinit hs_probe(struct platform_device *pdev)
{
	...
	input_set_capability(ipdev, EV_KEY, KEY_ENTER);
