...
//[KB62] ===> BugID#932 : SMD RPC log, added by Jimmy@CCI
...
//[KB62] ===> BugID#932 : SMD RPC log, added by Jimmy@CCI
	...
	{
		printk("[SMD]%s():receive RPC call:xid=%u, prog=0x%X, proc=0x%X\n", __func__, cpu_to_be32(rq->xid), cpu_to_be32(rq->prog), cpu_to_be32(rq->procedure));
	}
#endif // #ifdef CONFIG_CCI_PM_LOG
//[KB62] ===> BugID#932 : SMD RPC log, added by Jimmy@CCI
		/* RPC CALL */
