...
#if 1
static int bt_uart_suspend(struct platform_device *pdev, pm_message_t state)
{
	int retval = 0;
	/* BT Power Off */
	...
	if (msm_hs_tx_empty(bsi->uport)) {
		/*Deactivating UART */
		printk(KERN_INFO "Bluetooth UART1 -- Power Down\n");
		hsuart_power(0);
		/* UART clk is not turned off immediately. Release
		 * wakelock after 500 ms.
		 */
...
static int bt_uart_resume(struct platform_device *pde)
{
	/* BT Power Off */
	if (gpio_get_value(16) == 0)
		return 0;
	/*Activating UART */
	printk(KERN_INFO "Bluetooth UART1 -- Power Up\n");
			hsuart_power(1);
...
#if 1
	.suspend = bt_uart_suspend,
	.resume = bt_uart_resume,
#endif
	.driver = {
		.name = "bluesleep",
		.owner = THIS_MODULE,
