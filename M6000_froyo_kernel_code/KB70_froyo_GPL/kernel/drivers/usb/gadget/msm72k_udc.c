...
//Robert, 20100916, KB62_CR658 : Porting froyo (BSP6036)
extern void kb60_android_charger_usb_change(int plugin, int type);
...
//B: Robert, 20100916, KB62_CR658 : Porting froyo (BSP6036)
//Notify charging status to battery driver, weijiun_ling
static unsigned usb_chg_type_to_battery(enum chg_type usb_chg_type)
{
    if(usb_chg_type == USB_CHG_TYPE__SDP)
    {
        return 2;    //2=USB
    }
    else if(usb_chg_type == USB_CHG_TYPE__WALLCHARGER)
    {
        return 1;    //1=AC
    }
...
static void usb_chg_detect(struct work_struct *w)
{
	...
	struct msm_otg *otg = to_msm_otg(ui->xceiv); //Robert, 20100920, KB62_CR673 : Sync to BSP6040
	...
	atomic_set(&otg->chg_type, temp); //Robert, 20100920, KB62_CR673 : Sync to BSP6040

	//Robert, 20100916, KB62_CR658 : Porting froyo (BSP6036)
	//Notify charging status to battery driver, weijiun_ling
	chgNotify = usb_chg_type_to_battery(temp);
	kb60_android_charger_usb_change(1, chgNotify);
	printk("[USB]: OFFLINE -> ONLINE, chgNotify:%d\n",chgNotify);
    ...
    ////////////////////////////////////////////////////////////////////////////////////////////////////////////////
	//B: Robert, 20100916, KB62_CR658 : Porting froyo (BSP6036)
    // Charger plug-in when suspended, wakeup testing
    if (temp == USB_CHG_TYPE__WALLCHARGER/* || temp == USB_CHG_TYPE__SDP*/) 
    {
        int fd;
        fd = sys_open("/sys/power/state", O_RDWR, 0);
        if( fd == 0 )
            fd = sys_open("/sys/power/state", O_RDWR, 0); // for first time booup
        if( fd > 0)
        {
            sys_write(fd, "on", 2);
            sys_close(fd);
	 ...
	 * */
	if (temp == USB_CHG_TYPE__WALLCHARGER) {
		//B: Robert, 20100920, KB62_CR673 : Sync to BSP6040
		...
		case USB_STATE_ONLINE:
			...
			if (flags & USB_FLAG_VBUS_OFFLINE) {
				...
				if (temp != USB_CHG_TYPE__INVALID)
				{
					...
					//Notify charging status to battery driver, weijiun_ling
					chgNotify = usb_chg_type_to_battery(temp);
					kb60_android_charger_usb_change(0, chgNotify);
					printk("[USB]: ONLINE -> OFFLINE, chgNotify:%d\n",chgNotify);
					//E: Robert, 20100916, KB62_CR658
				...
				otg->reset(ui->xceiv); //Robert, 20100920, KB62_CR673 : Sync to BSP6040
				otg_set_suspend(ui->xceiv, 1);
...
static int msm72k_udc_vbus_session(struct usb_gadget *_gadget, int is_active)
{
	...
	#if 0  //B: Robert, 20101020, KB62_CR871 : Fix plug in/out USB cable quickly cause sometimes system can't suspend
		wake_lock(&ui->wlock);
	#else
		wake_lock_timeout(&ui->wlock, 3*HZ);
	#endif //E: Robert, 20101020, KB62_CR871
...
//B: Robert, 20100916, KB62_CR658 : Porting froyo (BSP6036)
static int en_modem = 0;
static int pid = 0x0284;

static int usb_modem_read_proc(char *page, char **start, off_t off, int count,	int *eof, void *data)
{
    char *out = page;
    int  len;
...
static int __init init(void)
{
    ...
    d_entry = create_proc_entry("usb_modem", S_IRUGO | S_IWUSR | S_IWGRP, NULL);
    if (d_entry) {
        d_entry->read_proc = usb_modem_read_proc;
        d_entry->write_proc = NULL;
        d_entry->data = NULL;
    }
    if( (pid == 0x0284) || (pid == 0x027b) )
        en_modem = 1;
	//E: Robert, 20100916, KB62_CR658
