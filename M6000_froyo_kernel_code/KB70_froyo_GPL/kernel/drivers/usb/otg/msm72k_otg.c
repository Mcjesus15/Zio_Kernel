...
static int msm_otg_suspend(struct msm_otg *dev)
{
	...
	int vbus = 0;
	unsigned otgsc;
	enum chg_type curr_chg = atomic_read(&dev->chg_type); //Robert, 20100920, KB62_CR673 : Sync to BSP6040
	...
	if (is_b_sess_vld() && !is_host() &&
				curr_chg != USB_CHG_TYPE__WALLCHARGER && //Robert, 20100920, KB62_CR673 : Sync to BSP6040
