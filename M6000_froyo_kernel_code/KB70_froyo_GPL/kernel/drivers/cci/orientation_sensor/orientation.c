 ...
 */
  /*
 *  orientation_sensor driver
 *  the Copyright (c) is claimed by Yamaha
 *  the location of code is .../kernel/drivers/cci/orientation_sensor
...
#if earlySuspend
static void ori_early_suspend(struct early_suspend *h);
static void ori_late_resume(struct early_suspend *h);
#endif
//HuaPu: end
...
static void ori_early_suspend(struct early_suspend *h)
{
	...
	printk(KERN_INFO "[Orientation]HuaPu>>>%s\n", __func__);
...
//HuaPu: begin /20101105 /BugID: 970 [KB62][Froyo][G-sensor] acceleration_sensor, geomagnetic_sensor and orientation_sensor fixing.
	...
	data->early_suspend.suspend = ori_early_suspend;
	data->early_suspend.resume = ori_late_resume;
