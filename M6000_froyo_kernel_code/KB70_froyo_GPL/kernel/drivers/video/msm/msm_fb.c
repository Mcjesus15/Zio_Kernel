...
int msm_fb_cursor(struct fb_info *info, struct fb_cursor *cursor)
{
	...
	value = mfd->cursor_update(info, cursor);
	MSG_FUN_EXIT;
	return value;
}
...
static int msm_fb_suspend_sub(struct msm_fb_data_type *mfd)
{
	int ret = 0,count = 0; //james@cci added count
			...
			if (mfd->panel_info.lcd.hw_vsync_mode) {
				...
				do {
					vh_pending = mfd->vsync_handler_pending;
                                        ...
                                        printk("%s vh_pending = %d\n",__FUNCTION__,vh_pending);
                                        count++;
                                        if(count++ > 10) {
                                                vh_pending = 0; //james@cci added
                                                printk("%s for let vh_pending to 0 for count to 10\n",__FUNCTION__);
                                        }
...
static int msm_fb_blank(int blank_mode, struct fb_info *info)
{
	...
	int value;
	MSG_FUN_ENTER;
	value = msm_fb_blank_sub(blank_mode, info, mfd->op_enable);
	MSG_FUN_EXIT;
...
static int msm_fb_register_driver(void)
{
	...
	MSG_FUN_ENTER;
	value = platform_driver_register(&msm_fb_driver);
	MSG_FUN_EXIT;
	return value;
}
