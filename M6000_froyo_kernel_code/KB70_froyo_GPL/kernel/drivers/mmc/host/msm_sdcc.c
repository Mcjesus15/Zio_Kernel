...
static struct wake_lock sdcc_wakelock;
 
#define CARD_REINSERT_DURATION (7 * HZ)
#define CARD_STANDBY_INSERT_DURATION (10)
//E: Robert, 20101006, KB62_CR766
...
msmsdcc_check_status(unsigned long data)
{
	...
	if (!host->plat->status) {
		...
		if (status ^ host->oldstat) {
			...
			if (status) {
				if (duration < CARD_REINSERT_DURATION)
					duration = CARD_REINSERT_DURATION - duration;
				else
					duration = CARD_STANDBY_INSERT_DURATION;
			} else
				duration = 0;

			wake_lock_timeout(&sdcc_wakelock, duration + 2*HZ);
	...
	if (plat->status_irq) {
		...
		if (ret) {
			pr_err("Unable to get slot IRQ %d (%d)\n",
			       plat->status_irq, ret);
			goto clk_disable; //Robert, 20101006, KB62_CR766 : Fix unexpectedly remove SD card issue
		}
...
static int __init msmsdcc_init(void)
{
	...
	wake_lock_init(&sdcc_wakelock, WAKE_LOCK_SUSPEND, "sdcc_present");
