...
//B: Robert, 20100719, TK11330 : CPU sleep control during Wi-Fi on
static unsigned int msmsdcc_wifi_keep = 1;
static void msmsdcc_wifi_keep_on(unsigned int on)
{
    if( on )
        msmsdcc_wifi_keep = 1;
    else
...
EXPORT_SYMBOL(msmsdcc_wifi_keep_on);
//E: Robert, 20100719, TK11330

//B: Robert, 20100811, KB62_CR605 : Implement OOB interrupt
static struct msmsdcc_host *gWiFiHost = NULL;
static void msmsdcc_wifi_hclk_pclk_on(unsigned int on)
{
    unsigned long		flags;
    ...
    spin_lock_irqsave(&gWiFiHost->lock, flags); //Robert, 20101123, KB62_CR1051 : Fix multiple instances WiFi download to SD card freeze
    if( on )
    {
        if (!gWiFiHost->clks_on) {
			if (!IS_ERR(gWiFiHost->pclk))
                clk_enable(gWiFiHost->pclk);
            clk_enable(gWiFiHost->clk);
            gWiFiHost->clks_on = 1;
            //printk("//msmsdcc_wifi_hclk_pclk_on\n");
        }
    }
    else
    {
        if (gWiFiHost->clks_on) {
            clk_disable(gWiFiHost->clk);
			if (!IS_ERR(gWiFiHost->pclk))
                clk_disable(gWiFiHost->pclk);
            gWiFiHost->clks_on = 0;
            //printk("//off\n");
        }
    }
    spin_unlock_irqrestore(&gWiFiHost->lock, flags); //Robert, 20101123, KB62_CR1051 : Fix multiple instances WiFi download to SD card freeze
}
EXPORT_SYMBOL(msmsdcc_wifi_hclk_pclk_on);
//E: Robert, 20100811, KB62_CR605
...
msmsdcc_pio_irq(int irq, void *dev_id)
{
	...
	//B: Robert, 20100618, KB62_CR339 : Sync to latest AB60 SD drivers
	/*AB_KB workaround if host->pio.sg is null make kmap_atomic die*/
...
//B: Robert, 20100811, KB62_CR605 : Implement OOB interrupt
static void msmsdcc_set_clk_on(struct mmc_host *mmc)
{
    unsigned long		flags;
	struct msmsdcc_host *host = mmc_priv(mmc);
    ...
    spin_lock_irqsave(&host->lock, flags); //Robert, 20101123, KB62_CR1051 : Fix multiple instances WiFi download to SD card freeze
    if (!host->clks_on) {
        //printk("//on\n");
		if (!IS_ERR(host->pclk))
            clk_enable(host->pclk);
    ...
    spin_unlock_irqrestore(&host->lock, flags); //Robert, 20101123, KB62_CR1051 : Fix multiple instances WiFi download to SD card freeze
...
#endif
    .set_clk_on = msmsdcc_set_clk_on, //Robert, 20100811, KB62_CR605 : Implement OOB interrupt
/* //Robert, 20100618, KB62_CR339 : Sync to latest AB60 SD drivers
#ifdef CONFIG_MMC_AUTO_SUSPEND
...
// Broadcom WiFi power
	...
	msleep(1000);
	printk("Enter wifi-%s, WLAN SDIO probe GPIO109 = %d\n", __func__, gpio_get_value(109));

	/* must have platform data */
	if (!plat) {
		pr_err("wifi-%s: Platform data not available\n", __func__);
		ret = -EINVAL;
		goto out;
	}
	...
	if (pdev->resource == NULL || pdev->num_resources < 3) {
		pr_err("wifi-%s: Invalid resource\n", __func__);
		return -ENXIO;
	}
	...
	gWiFiHost = host; //Robert, 20100811, KB62_CR605 : Implement OOB interrupt
	...
	if (plat->sdiowakeup_irq) {
		ret = request_irq(plat->sdiowakeup_irq,
			...
			IRQF_SHARED | IRQF_TRIGGER_RISING, //Robert, 20100719, TK11330 : CPU sleep control during Wi-Fi on
...
*/
 ...
 host_free:
	mmc_free_host(mmc);
 out:
    gWiFiHost = NULL; //Robert, 20100811, KB62_CR605 : Implement OOB interrupt
	return ret;
}
...
static int msmsdcc_remove(struct platform_device *pdev)
{
	...
	printk("== wifi - Removing SDCC2 device = %d ==\n", pdev->id);
	plat = host->plat;
	...
	free_irq(host->irqres->start, host); //Robert, 20100914, KB62_CR633 : Fix WiFi On/Off issue at BSP6036
...
*/
	...
	printk(" wifi-%s, WLAN SDIO EXIT GPIO109 = %d\n", __func__, gpio_get_value(109));
...
*/
	if (mmc) {
        ...
        //B: Robert, 20100719, TK11330 : CPU sleep control during Wi-Fi on
        if( !msmsdcc_wifi_keep )
			rc = mmc_suspend_host(mmc, state);
        else {
            rc = 0;
            printk(KERN_ERR "//HOST_WAKE_WL [%d]\n", gpio_get_value(31));
            printk(KERN_ERR "//WL_WAKE_Host [%d]\n", gpio_get_value(17));
            printk(KERN_ERR "//WL_RESET [%d]\n", gpio_get_value(109));
        }
...
static struct platform_driver msmsdcc_driver = {
	...
	.driver		= {
		.name	= "wifi_msm_sdcc", //Robert, 20100618, KB62_CR339 : Sync to latest AB60 SD drivers
	},
};
